...
const Header = ({ setListImages, setIsLoading }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsLoading(true);
            const response = await fetch("https://geniart-api.onrender.com/openai/images", {
                method: "POST",
                headers: {
            ...
            <div className="container mx-auto max-w-5xl px-2 py-6 ">
                ...
                <div className="mt-14">
                    ...
                    <form className="relative my-8  max-w-md" onSubmit={handleSubmit}>
                        <input value={inputValue} onChange={handleInputChange} className=" indent-2 py-4 pr-[4.5rem] rounded-sm 
                        bg-gray-50 w-full md:max-w-md focus:ring-primary 
                        outline-none focus:ring-1" type="text" placeholder="" />
                        <button type="submit" className="absolute bottom-2.5 right-2.5 bg-primary
                        text-white px-2 py-2 rounded-sm ">
